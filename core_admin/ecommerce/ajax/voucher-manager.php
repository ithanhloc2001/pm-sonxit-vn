...
if (isset($ECOMMERCE_PAYMENT_METHODS) && is_array($ECOMMERCE_PAYMENT_METHODS)) {
    foreach ($ECOMMERCE_PAYMENT_METHODS as $key => $cfg) {
        ...
        $voucherPaymentOptions[] = ['key' => (string)$key, 'label' => $label];
    }
}
if (!$voucherPaymentOptions) {
    $voucherPaymentOptions = [['key' => 'cod', 'label' => 'COD']];
}
// Các phương thức vận chuyển GHN phổ biến, admin có thể nhập thủ công vào ô Shipping Methods khi tạo/sửa voucher
$ghnShippingOptions = [];
if (function_exists('defaultShippingMethodLabelMap')) {
    ...
    // Fallback trong trường hợp helper chưa tồn tại
    $ghnShippingOptions = [
        ['key' => 'ghn_nhanh', 'label' => 'GHN Nhanh'],
        ['key' => 'ghn_tiet_kiem', 'label' => 'GHN Tiết kiệm'],
        ['key' => 'ghn_hoa_toc', 'label' => 'GHN Hỏa tốc'],
        ['key' => 'tieu_chuan', 'label' => 'Tiêu chuẩn'],
    ];
}

function normalizeDiscountTarget(string $raw): string {
    return strtolower(trim($raw)) === 'shipping' ? 'shipping' : 'order';
}
function normalizeCsvInput($raw): string {
    $items = [];
    if (is_array($raw)) {
        $items = $raw;
    } else {
...
function normalizeIdListInput(string $raw): string {
    $ids = normalizeProductIds($raw);
    return $ids ? implode(',', $ids) : '';
}

// Hàm xây dựng chuỗi mô tả chi tiết cho voucher, dùng khi admin không nhập custom detail_text
function buildVoucherDetailText(array $data): string {
    $code = strtoupper(trim((string)($data['code'] ?? '')));
    $value = (float)($data['value'] ?? 0);
    $valueUnit = strtolower((string)($data['value_unit'] ?? 'fixed'));
    if ($valueUnit !== 'percent' && $valueUnit !== 'fixed') {
        $valueUnit = ($valueUnit === 'percent') ? 'percent' : 'fixed';
    }
    ...
    // Mệnh giá và trần giảm giá dùng chung chuẩn hoá (K / %)
    $valueLabel = format_voucher_value_label($valueUnit, $value);
    $maxLabel = ($max !== null && $max !== '') ? format_voucher_value_label($maxUnit, (float)$max) : '';

    if ($valueUnit === 'percent') {
        if ($valueLabel === 'Miễn phí') {
            $discountText = 'Mã ' . $code . ' miễn phí đơn hàng';
        } else {
            $discountText = 'Mã ' . $code . ' giảm ' . ($valueLabel !== '' ? $valueLabel : ($value . '%'));
        }
    } else {
    ...
    $minLabel = $min > 0 ? format_voucher_value_label($minUnit, $min) : '';
    $minText = $min > 0 && $minLabel !== ''
        ? (' cho đơn hàng hợp lệ từ ' . $minLabel)
        : ' cho đơn hàng hợp lệ';
    $endText = $endAt !== '' ? (' HSD: ' . $endAt . '.') : '';
    $limitText = ($maxUses !== null && $maxUses !== '') ? ' Số lượng có hạn.' : ' Số lượng có hạn.';

    return $discountText . $minText . ' ' . $endText . $limitText
        . ' Không áp dụng với đơn hàng từ Người bán không đủ điều kiện hưởng ưu đãi vận chuyển.';
}
// ===== ACTIONS =====
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save') {
        ...
        foreach ((array)$targetsRaw as $t) {
            $t = normalizeDiscountTarget((string)$t);
        ...
        if ($voucher_template === 'only_category_discount' && $apply_scope === 'products' && $apply_category_ids === '') {
        ...
        $apply_user_ids = normalizeIdListInput((string)($_POST['apply_user_ids'] ?? ''));
        $exclude_product_ids = normalizeIdListInput((string)($_POST['exclude_product_ids'] ?? ''));
        $payment_methods = normalizeCsvInput($_POST['payment_methods'] ?? '');
        $shipping_methods = normalizeCsvInput($_POST['shipping_methods'] ?? '');
        ...
        if ($detail_text === '') {
            $detail_text = buildVoucherDetailText([
                'code' => $code,
                'value' => $value,
...
$qProducts = $ithanhloc->query("SELECT p.id, p.product_name, {$skuExpr}, p.category_id FROM ecommerce_product p ORDER BY p.product_name ASC, p.id DESC");
if ($qProducts) {
    $p_list = [];
    $p_ids = [];
    ...
    if ($p_ids) {
        ...
        // Load Variants
        $qVariants = $ithanhloc->query("SELECT id, product_id, group_id, variant_name, volume_label, sku_variant FROM ecommerce_product_variants WHERE product_id IN ($p_ids_str) ORDER BY id ASC");
        if ($qVariants) {
            while ($v = $qVariants->fetch_assoc()) {
                ...
                $v_data = [
                    'id' => (int)$v['id'],
                    'name' => trim((string)$v['variant_name'] . ' ' . (string)$v['volume_label']),
...
if (!function_exists('format_voucher_value_label')) {
    function format_voucher_value_label($type, $val, $max = null) {
        $hasMax = ($max !== null && $max !== '');
        $maxVal = $hasMax ? (float)$max : 0.0;
        if ($type === 'percent') {
            $main = ($val >= 100) ? 'Miễn phí' : ('Giảm ' . rtrim(rtrim(number_format($val, 1, '.', ''), '0'), '.') . '%');
            if ($hasMax && $maxVal > 0) {
...
function VoucherValueText($c) {
    $value_unit = strtolower((string)($c['value_unit'] ?? 'fixed'));
    $value = (float)($c['value'] ?? 0);
    $max = $c['max_discount'] ?? null;
    $hasMax = ($max !== null && $max !== '');
    $maxVal = $hasMax ? (float)$max : 0.0;

    if ($value_unit === 'percent') {
        $main = ($value >= 100) ? 'Miễn phí' : ('Giảm ' . rtrim(rtrim(number_format($value, 1, '.', ''), '0'), '.') . '%');
        if ($hasMax && $maxVal > 0) {
            $main .= ' (tối đa ' . formatMoney($maxVal) . 'đ)';
        }
        return $main;
...
function VoucherTimeSummary($c) {
    ...
    $fmt = static function(string $v): string {
        if ($v === '') return '';
        // yyyy-mm-dd hh:mm:ss -> lấy tới phút
        $v = str_replace('T', ' ', $v);
        $v = preg_replace('/\.[0-9]+$/', '', $v); // bỏ microseconds nếu có
        return preg_replace('/^(\d{4}-\d{2}-\d{2} \d{2}:\d{2}).*$/', '$1', $v);
    };

    $s = $fmt($start);
    $e = $fmt($end);

    if ($s !== '' && $e !== '') return $s . ' đến ' . $e;
    if ($e !== '') return 'Đến ' . $e;
    if ($s !== '') return 'Từ ' . $s;
